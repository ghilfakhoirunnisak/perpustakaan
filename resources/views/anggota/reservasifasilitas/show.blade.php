    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card shadow">
            <div class="card-body">
                ...
                <div class="mb-3">
                    <label class="form-label fw-semibold">Catatan Verifikator</label>
                    <div id="catatan_list" class="form-control bg-light" style="min-height: 40px;">
                        <em>Memuat...</em>
                    </div>
                </div>

                <div class="text-end">
                    <a href="{{ route('anggota.reservasifasilitas.index') }}" class="btn btn-secondary">Kembali</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function loadDetail() {
            try {
                ...
                // Tampilkan catatan verifikator jika ada
                const catatanBox = document.getElementById('catatan_list');
                catatanBox.innerHTML = '';

                if (data.log_approval && data.log_approval.length > 0) {
                    data.log_approval.forEach(log => {
                        const catatan = document.createElement('div');
                        catatan.innerHTML = `<strong>Level ${log.level}:</strong> ${log.catatan ?? 'Tidak ada catatan.'}`;
                        catatanBox.appendChild(catatan);
                    });
                } else {
                    catatanBox.innerHTML = '<em>Tidak ada catatan</em>';
                }
